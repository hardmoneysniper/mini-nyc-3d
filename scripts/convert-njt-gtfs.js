...
function main() {
    ...
    for (const route of routes) {
        ...
        // stopping at very different subsets of its stations — e.g. a
        // Hackettstown-Hoboken local and a short-turn Bay Street-Hoboken
        // trip can both use shape_id "2" on NJT's Montclair-Boonton Line,
        // 26 stops vs 6. Picking an arbitrary trip per shape_id silently
        // drops whichever stations only appear on a longer trip sharing
        // that shape. Keep the trip with the most stop_times rows instead
        // — the fullest/local pattern is a superset of every shorter trip
        // on the same shape in practice, so this maximizes real station
        // coverage without changing which shape's geometry gets used.
