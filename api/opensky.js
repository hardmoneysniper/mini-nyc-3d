/**
 * OpenSky Network proxy — OAuth2 authenticated, credit-efficient design:
 *
 *   • Auth:     OAuth2 client credentials → Bearer token, cached at module level
 *               (survives Vercel warm starts). Falls back to anonymous if env vars missing.
 *   • Credits:  4000/day authenticated; our bbox (0.7°×1.5°=1.05sq°) costs 1 credit/call.
 *               s-maxage=30 → 2880 calls/day (72% of daily quota).
 *   • Routes:   api.adsbdb.com callsign lookup (free, no auth) for each aircraft
 *               near an airport. Returns origin & destination airport with name.
 *               Route data drives arrival/departure classification — more accurate
 *               than vertical-rate alone (excludes transiting aircraft).
 *   • ATIS:     aviationweather.gov METAR as fallback; live aircraft headings
 *               override METAR when final-approach traffic is present.
 *   • Response: {atisData, flightData} only — openskyStates not sent to client.
 *
...
// Module-level token cache — persists across Vercel warm-start invocations
...
async function getToken() {
    ...
    try {
        ...
        if (!res.ok) { console.error('[token] fetch failed:', res.status); return null; }
        const data = await res.json();
        _token = data.access_token;
        _tokenExpiresAt = now + ((data.expires_in || 1800) - 30) * 1000;
        return _token;
    } catch (e) {
...
function buildFlightData(candidates, routeMap, nowMs) {
    ...
    for (const {s, nearestIcao, nearestIata, nearestDist, alt, id,
        ...
        const routeIsArrival   = route && route.destIata   === nearestIata;
        const routeIsDeparture = route && route.originIata === nearestIata;
        const hasRoute = route != null;

        const isArrival = routeIsArrival
            || (!hasRoute && vertRate < -2.0 && nearestDist <= ARRIVAL_DIST);
        const isDeparture = routeIsDeparture
            || (!hasRoute && vertRate > 2.0 && nearestDist <= DEPARTURE_DIST);

        // Skip aircraft whose known route has nothing to do with this airport
        if (hasRoute && !routeIsArrival && !routeIsDeparture) continue;

        if (isArrival && nearestDist <= ARRIVAL_DIST && alt > 50 && alt <= 700) {
            const timeToLandMs = estimateTimeMs(nearestDist, alt, vertRate, APPROACH_KMH);
            ...
            if (heading != null && rwys) {
                ...
                headingVotes[nearestIcao].landing[rwy] = (headingVotes[nearestIcao].landing[rwy] ?? 0) + 1;
            }

            const flight = {id: `${id}-arr`, n: cs, ar: nearestIata, sat: toEastern(nowMs + timeToLandMs)};
            if (route) flight.or = airportObj(route.originIata, route.originName);
            flights.push(flight);

        } else if (isDeparture && nearestDist <= DEPARTURE_DIST && alt <= 700) {
            const timeAloftMs = alt > 50
                ? estimateTimeMs(nearestDist, alt, vertRate, CLIMB_KMH)
                : 30_000;

            if (heading != null && rwys) {
                const rwy = bestRunway(heading, rwys.departure);
                if (!headingVotes[nearestIcao]) headingVotes[nearestIcao] = {landing: {}, departure: {}};
                headingVotes[nearestIcao].departure[rwy] = (headingVotes[nearestIcao].departure[rwy] ?? 0) + 1;
            }

            const flight = {id: `${id}-dep`, n: cs, dp: nearestIata, sdt: toEastern(nowMs - timeAloftMs)};
...
export default async function handler(req, res) {
    ...
    // 30s CDN cache → 2880 calls/day = 72% of 4000-credit/day authenticated quota
    ...
    try {
        ...
        const statesRes = await fetch(STATES_URL, {
            headers: {'User-Agent': 'mini-nyc-3d/0.1.0', ...authHeader}
        });
        ...
        const states = (data.states || [])
            ...
            .map(s => ({
                ...
                callsign: (s[1] || '').trim(),
                lon:      s[5], lat:      s[6],
                baroAlt:  s[7], onGround: s[8],
                heading:  s[10], vertRate: s[11],
                geoAlt:   s[13]
            }));

        const candidates = filterCandidates(states);
        const [routeMap] = await Promise.all([
            lookupRoutes(candidates),
            fetchAtis()
        ]);
